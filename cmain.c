#include "cmain.h"

int main(int argc, char *argv[])
{
    gtk_init(&argc, &argv); // Initialize GTK
    GtkWidget *win = gtk_window_new(GTK_WINDOW_TOPLEVEL); // Create window widget
    g_signal_connect(win, "delete_event", G_CALLBACK(end_program), NULL); // Connect callback to window
//    gtk_widget_show(win); // Show window

    GtkWidget *btn = gtk_button_new_with_label("Hello? World?"); // Create button widget

    g_signal_connect(btn, "clicked", G_CALLBACK(end_program), NULL); // Connect callback to button

    gtk_container_add(GTK_CONTAINER(win), btn); // Add button to window
    gtk_widget_show_all(win); // Show window container and thusly button

    gtk_main(); // Run GTK main loop

    return 0;
}

void end_program(GtkWidget *wid, gpointer ptr)
{
    gtk_main_quit(); // Quit GTK
}
